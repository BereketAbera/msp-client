...
export class DepositeWithNewCreditCardComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.zipCodeHints$ = this.paymentForm.get("zipCode").valueChanges.pipe(
      ...
      distinctUntilChanged(),
      switchMap((searchTXT) => this.zipCodeService.listZipcods(searchTXT))
    );
  }
  getErrorMessage() {}
  getCardType(cardNum) {
  ...
  onSubmit() {
    if (
      ...
      this.paymentForm.get("cvc").value.toString().length >= 3 &&
      this.paymentForm.get("cvc").value.toString().length <= 4
    ) {
