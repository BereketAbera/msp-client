...
export class BuyerDepositComponent implements OnInit {
  ...
  constructor(private cmpntFctryResolver:ComponentFactoryResolver,  private router: Router, private route: ActivatedRoute) { }
  ...
  loadPaymentWithCreditCardComponent(){
    ...
    let componentFactory = this.cmpntFctryResolver.resolveComponentFactory(DepositeWithSavedCreditCardComponent);

    let viewContainerRef = this.paymentType.viewContainerRef;
    viewContainerRef.clear();
    let componentRef = viewContainerRef.createComponent(componentFactory);
    (<DepositeWithSavedCreditCardComponent>componentRef.instance).data = this.paymentInfo;
    (<DepositeWithSavedCreditCardComponent>componentRef.instance).payWith.subscribe(type=>{
        this.loadPaymentWithNewCreditCardComponent();
    });
  ...
  loadPaymentWithNewCreditCardComponent() {
    ...
    let componentFactory = this.cmpntFctryResolver.resolveComponentFactory(DepositeWithNewCreditCardComponent);
