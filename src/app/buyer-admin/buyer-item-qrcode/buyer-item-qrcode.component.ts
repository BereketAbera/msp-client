...
export class BuyerItemQrcodeComponent implements OnInit {
  ...
  code = "xxx-xxx";
  constructor(private router:Router,public snackBar: MatSnackBar, public dialog: MatDialog, private route: ActivatedRoute, private trnsService: TransactionService) { }

  ngOnInit() {
    let id = +this.route.snapshot.paramMap.get('id');
    this.route.data
      .subscribe((data: { transaction: Transaction, supplier: Supplier, transStatus: TransactionStatus }) => {

        this.transaction = data.transaction;
        if(this.transaction.codePool && this.transaction.codePool.code && this.transaction.codePool.code.length == 6)
           this.code = this.transaction.codePool.code.slice(0,3) + "-" + this.transaction.codePool.code.slice(3,6);
