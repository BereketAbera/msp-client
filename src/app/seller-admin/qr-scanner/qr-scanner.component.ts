...
export class QrScannerComponent implements OnInit {
  ...
  constructor(private authService:AuthService,public dialog: MatDialog,private transactionService:TransactionService) { }

  ngOnInit() {
    
    //if(!this.authService.accountCanScan())
        //this.showError = true;
        const typeahead = fromEvent(this.myCodeElementHTML.nativeElement, 'keyup').pipe(
...
checkCode(){
  //this.stopScanner()
    ...
    let ordCode  = this.code.value.replace("-","");
    let dialogRef = this.dialog.open(RequestCodeConfirmationComponent,{data: {code:ordCode}});
            dialogRef.afterClosed().subscribe(result => {
              if(result){
                ...
                //this.isScanning = false;
                //this.video.pause();
                this.transactionService.processTransactionQRCdCode({code:ordCode,qrCode:result.qrCode}).subscribe(
                   result=>{
                     if(result['success']){
      ...
      if (code) {
        ...
        try {
          ...
          if (cstInfo) {
            ...
            this.isScanning = false;
            let dialogRef = this.dialog.open(RequestConfirmationComponent,{data: cstInfo});
            dialogRef.afterClosed().subscribe(result => {
              if(result){
                this.isScanning = false;
                this.video.pause();
                this.transactionService.processTransactionQRCode(cstInfo).subscribe(
                   result=>{
                     if(result['success']){
                     this.displayChk = "block";
                     this.displayScanner = "none";
