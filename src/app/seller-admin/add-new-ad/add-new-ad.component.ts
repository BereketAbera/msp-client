...
export class AddNewAdComponent implements OnInit {
  ...
  pickupStartInitTime = new Date(moment(this.offerStartInitTime).add(10, 'm').valueOf());
  pickupEndInitTime = new Date(moment(this.offerEndInitTime).add(40, 'm').valueOf());
  ...
  options: string[];
  hhmm: string[] = [];
  hhlst: string[] = ["01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12"];
  pmam = ["PM", "AM"];
  ...
  constructor(private authService:AuthService,  public snackBar: MatSnackBar, public dialog: MatDialog, private route: ActivatedRoute, private router: Router, private productService: ProductService, private fb: FormBuilder) { }

  ngOnInit() {
    this.productForm.get('normalPrice').valueChanges.pipe(
      debounceTime(5000)
    ...
    this.route.data
      .subscribe((data: { shops: Shop[], categories: Category[], markup: number,pictures:Picture[] }) => {
        ...
        if(this.shops.length < 1){
          ...
          dialogRef.afterClosed().subscribe(result => {
            this.router.navigate(["../../shops/newshp"],{relativeTo:this.route});
          });
    ...
    this.productForm.get('categoryId').valueChanges.subscribe(value => {
      ...
      if (value == "1") {
        this.options = ["Sandwich", "Pizza", "Fried chicken", "Cheeseburger", "Special Drinks","BBQ Meats","Salad","Pasta","All you can eat"];
      } else if (value == "2") {
          ...
          this.productService.createProduct(product).subscribe(
            res => {
              if (res['success']) {
                ...
                let snackBarRef = this.snackBar.open("Successfuly saved", "Add More", {
                  duration: 2000,
                });
  ...
  checkFormValidity() {
    ...
    let offerStartDate = moment(this.productForm.get('offerStartDate').value).format("YYYY-MM-DD");
    ...
    let offerEndDate = moment(this.productForm.get('offerEndDate').value).format("YYYY-MM-DD");;
