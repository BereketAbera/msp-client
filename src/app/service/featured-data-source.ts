...
export class FeaturedDataSource extends DataSource<Product | undefined> {
    ...
    constructor(private authService:AuthService, private transactionService: ProductService,private catagoryId:number|string) {
        super();
    ...
    loadTransactions(
        ...
        pageIndex: number,
        pageSize: number) {
        /*this.transactionService.listProductsHomeA(this.authService.currentLat,this.authService.currentLong,subCatagoryId,filter, sortDirection,
            pageIndex, pageSize).subscribe(products => {
                if(products && products.length >0){
                    this.count += products.length;
                    //this.cachedData = Array.from<Product>({ length: this.count});
                    this.fetchedPages.add(pageIndex);
                    if(this.count <= 5)
                        this.cachedData.splice(pageIndex * products.length, products.length,...products);
                    else
                        this.cachedData.splice(this.cachedData.length,0,...products);
                    
                }else{
                    this.isEndOfProduct = true;
                    this.cachedData.splice(15);
                }
