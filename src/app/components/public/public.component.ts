...
export class PublicComponent implements OnInit {
  ...
  backingItems = ['/assets/image/bnr1.jpg?ke=' + this.dtNow, '/assets/image/bnr2.jpg?ke=' + this.dtNow, '/assets/image/bnr3.jpg?ke=' + this.dtNow];
  actualIndex = 0;
  selectedIndex = 0;
  componentRef:any;
  locations = [
  ...
  getMyLocation() {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition((position) => {
       ...
       console.log("lat=" + this.authService.currentLat + " lng =" + this.authService.currentLong);
      },err=>{
